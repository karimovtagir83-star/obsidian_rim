...
Дублирование здесь — **осознанная плата за изоляцию**. Попытка вынести общий `ProcessingModule` в BuildingBlocks сразу создала бы связь «все модули зависят от общего кода регистрации», и любое изменение под нужды одного модуля ломало бы остальные. Это тот случай, когда DRY проигрывает — см. [[DRY, KISS, YAGNI и когда они врут]].
