...
Логика физически лежит в одном классе, а данные — в другом. Каждое изменение структуры `Order` (переименовали свойство, вынесли адрес, поменяли модель линий) ломает `ShippingCalculator`, хотя правила доставки не менялись. Это нарушение [[Закон Деметры и Tell, Don't Ask]]: цепочки `order.Lines[i].Product.Dimensions` привязывают вызывающего к внутреннему устройству двух-трёх уровней объектов. Формально: связность (cohesion) низкая, связанность (coupling) высокая — см. [[Связанность и связность (coupling, cohesion)]].
...
Guard clauses (ранние выходы) выпрямляют поток: сначала все причины не продолжать, затем — основной сценарий на нулевом уровне вложенности. Подробнее — [[Защитное программирование и guard clauses]].
...
Обратная сторона правила: как только третий случай появился — абстрагируй, не тяни. Дублирование, которое уже втрое, начинает расходиться версиями. Про баланс — [[DRY, KISS, YAGNI и когда они врут]].
